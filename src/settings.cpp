...
unordered_map<string, string> settings::db_settings;
...
auto settings::load_setting() -> void {
    ...
    try {
        ...
        for (Value::ConstMemberIterator it = doc["db_settings"].MemberBegin();
             it != doc["db_settings"].MemberEnd(); ++it) {
            db_settings.emplace(std::pair<string, string>(it->name.GetString(), it->value.GetString()));
        }
